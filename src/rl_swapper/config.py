...
DEFAULT_INSTALLATIONS = {
    "EpicGames": Path(r"C:\Program Files\Epic Games\rocketleague\TAGame\CookedPCConsole"),
    "Steam": Path(r"D:\SteamLibrary\steamapps\common\rocketleague\TAGame\CookedPCConsole")
}
...
class AppSettings:
    ...
    _rl_source_dir: str = DEFAULT_RL_SOURCE_DIR
    _installations: dict[Literal["EpicGames", "Steam"], Path] = field(
        default_factory=lambda: DEFAULT_INSTALLATIONS.copy() #type: ignore
    )
    _shipped_data_dir: str = str(SHIPPED_DATA_DIR)
    _decryption_work_dir: str = DEFAULT_DECRYPTION_WORK_DIR # for decrypted files, used by rl_asset_swapper.py
    _workspaces_dir: str = str(USER_DATA_DIR / "workspaces")
    _legacy_runs_dir: str = str(USER_DATA_DIR / "swap_runs") # TODO remove
    _db_file: str = str(SWAPS_DB_FILE)
    
    @property
    def installations(self) -> dict[Literal["EpicGames", "Steam"], Path]:
        """Dict of installation names to their paths.
        
        Used to map swaps to either Epic Games or Steam installation.
        
        Keys: `'EpicGames'`, `'Steam'`"""
        return self._installations
    
    @property
    @deprecated("Use installations dict instead")
    def rl_source_dir_path(self) -> Path:
        return Path(self._rl_source_dir)
    
    @property
    def legacy_runs_path(self) -> Path:
        return Path(self._legacy_runs_dir)
    
    @property
    def db_file_path(self) -> Path:
        return Path(self._db_file)
    
    @property
    def workspaces_path(self) -> Path:
        return Path(self._workspaces_dir)

    @property
    def items_path(self) -> Path:
        return Path(self._shipped_data_dir) / "items.json"

    @property
    def keys_path(self) -> Path:
        return Path(self._shipped_data_dir) / "keys.txt"

    @property
    def keys_map_path(self) -> Path:
        return Path(self._shipped_data_dir) / "keys_map.json"
    ...
    def decryption_work_path(self) -> Path:
        return Path(self._decryption_work_dir)
